...
<template>
  <div class="h-[calc(100vh-4rem)] flex flex-col">
    <!-- Toolbar -->
    <div class="border-b border-border bg-background px-4 py-2 flex items-center justify-between gap-4 shrink-0">
      <div class="flex items-center gap-3">
        <Terminal class="w-5 h-5 text-primary" />
        <span class="font-semibold text-sm hidden sm:inline">Playground</span>
        <LanguageSelector :modelValue="selectedLanguage" @update:modelValue="onLanguageChange" />
      </div>
    ...
    <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
      ...
      <!-- Output Column -->
      <div class="md:w-[40%] border-t md:border-t-0 md:border-l border-border flex flex-col min-h-0">
        <!-- Stdin -->
        <div class="border-b border-border p-3 shrink-0">
          <label class="text-xs font-medium text-muted-foreground block mb-1">Standard Input (stdin)</label>
          <textarea
            v-model="stdin"
        ...
        <div class="flex-1 min-h-0 overflow-auto">
          <CodeRunner :code="code" :language="selectedLanguage" :stdin="stdin" />
        </div>
      </div>
    </div>
  </div>
